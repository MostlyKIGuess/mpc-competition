...
CONTROL_GRID = {
    "slow_speeds": [0.0, 0.2, 0.5, 0.8],
    "cruise_speeds": [0.3, 0.6, 1.0, 1.4, 1.8, 2.2, 2.7],
...
def candidate_controls(
    ...
    proportional_turn = clamp(3.0 * heading_error, -1.0, 1.0)

    # Stop-turn-move strategy: restrict speeds based on heading alignment
    speeds = CONTROL_GRID["cruise_speeds"]
    if distance_to_goal < float(MPC_CONFIG["near_goal_distance"]):
        speeds = CONTROL_GRID["slow_speeds"]

    # Extra restriction when near goal with heading mismatch
    if distance_to_goal < 0.8 and abs(heading_error) > 0.3:
        speeds = [0.0, 0.1]  # Nearly stop to allow turning
    ...
    turn_rates.add(round(proportional_turn - 0.3, 3))
    turn_rates.add(round(proportional_turn + 0.3, 3))
...
def evaluate_rollout(
    ...
    # Encourage stop-turn-move: penalize forward speed with large heading error
    initial_distance = float(np.linalg.norm(goal[:2] - state[:2]))
    final_heading_error = abs(angle_difference(goal[2], predicted_state[2]))

    # When approaching goal with heading mismatch, penalize forward motion
    if initial_distance < 1.5 and final_heading_error > 0.23:
        total_cost += 7.5 * requested_control[0] * final_heading_error
